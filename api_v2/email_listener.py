...
def handle_user_password_forgotten_event(user):
    ...
    send_email(user.name, user.email, "Reset your password", f"To reset your password, use this very secure code: "
                                                             f"{user.reset_code}.\nRegards, The DevNotes team")


def handle_user_upgrade_plan_event(user):
    # send a thank you email
    send_email(user.name, user.email, "Thank you", f"Thanks for upgrading, {user.name}! "
                                                   f"You're gonna love it. \nRegards, The DevNotes team")
